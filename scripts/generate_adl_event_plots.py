...
time_tick_interval = config["time_tick_interval"]
...
def plot_event(csv_dir, adl_event, instance_idx, pdf):
    # Parse all sensors
    ...
    axes[-1].xaxis.set_major_locator(mdates.SecondLocator(interval=time_tick_interval))
    ...
    plt.tight_layout(rect=(0, 0, 1, 0.94), h_pad=0.2)
    plt.suptitle(f"{adl_event} (instance {instance_idx+1})", y=0.99, fontsize=18)
    pdf.savefig(fig)
    plt.close(fig)
...
with PdfPages(pdf_path) as pdf:
    for adl_event, event_folders in adl_events.items():
        ...
        for idx, csv_dir in enumerate(csv_dirs):
            ...
            try:
                plot_event(csv_dir, adl_event, idx, pdf)
